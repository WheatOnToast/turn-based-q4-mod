...
static void Cmd_SetSelectedUnit_f(const idCmdArgs& args) {
	int UnitNumber = NULL;
	//UnitNumber = gameLocal.SelectedUnit;
}
...
void idGameLocal::InitConsoleCommands(void) {
	...
	*/
	...
	// RITUAL END
		...
		//q4_mod_sys_cmds
	cmdSystem->AddCommand("locate", Cmd_Locate_f, CMD_FL_GAME, "Print out player location");
	cmdSystem->AddCommand("setSelectedUnit", Cmd_SetSelectedUnit_f, CMD_FL_GAME, "Stores unit that was selected (shot)");
}
